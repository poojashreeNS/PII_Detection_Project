    ...
    if request.method == "POST":
        if len(request.files) < 2: #check if tab1
            ...
            if(request.form['generic-text'] != ''): # if input is text
                ...
                pii_files = request.files['generic-file']
            ...
            embeddings = np.array([get_embedding(text) for text in custom_input])

            # Store the embeddings in a vector database
            index = faiss.IndexFlatIP(embeddings.shape[1])
            index.add(embeddings)

            results = search(test_input, index, custom_input)
            for result in results:
                for item in result:
                    if item[1] > 0.85:
                        ...
                        continue
                break
            render_template('index.html', result=result, active_tab=active_tab)
    else:
        result = ''
    return render_template('index.html', result=result, active_tab=active_tab)
...
def search(queries, index, texts, k=1):
